...
def unknown(update: Update, context: CallbackContext):
    context.bot.send_message(
        chat_id=update.message.chat_id, text="Я не понимаю эту команду"
    )
...
def register(update: Update, context: CallbackContext):
    ...
    if users_count == 1:
        context.bot.send_message(
            chat_id=update.message.chat_id, text="Вы уже зарегистрированы"
        )
    else:
...
def ready(update: Update, context: CallbackContext):
    if len(gamers) == 2:
        context.bot.send_message(
            chat_id=update.message.chat_id, text="Игра уже идет..."
        )
        return

    username = (
        User.select().where(User.telegram_id == update.effective_user.id).get().username
    )
    ...
    if len(gamers) == 1 and gamers[0]["user_id"] == user_id:
        context.bot.send_message(
            chat_id=gamers[0]["chat_id"], text="Вы уже подключились к игре."
        )
        return
    ...
    if len(gamers) == 1:
        context.bot.send_message(
            chat_id=update.message.chat_id, text="Ожидаем противника..."
        )
...
def name(update: Update, context: CallbackContext):
    ...
    user.save()
    context.bot.send_message(
        chat_id=update.message.chat_id, text=f"Теперь вы будете известны как {new_name}"
    )
...
def status(update: Update, context: CallbackContext):
    if len(gamers) == 0:
        ...
        context.bot.send_message(
            chat_id=update.message.chat_id,
            text=f'{gamers[0]["username"]} ожидает противника',
        )
    elif len(gamers) == 2:
        context.bot.send_message(
            chat_id=update.message.chat_id,
...
def score(update: Update, context: CallbackContext, args):
    if len(gamers) == 1:
        context.bot.send_message(
            chat_id=update.message.chat_id, text="Вы еще не сыграли игру."
        )
    s: int
    try:
...
def error(update: Update, context: CallbackContext):
    context.bot.send_message(
        chat_id=update.message.chat_id, text="Произошла ошибка обратитесь к логам"
    )
